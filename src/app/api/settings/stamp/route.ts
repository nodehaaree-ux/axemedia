...
export async function POST(req: Request) {
  try {
    ...
    if (existing) {
      await prisma.companySettings.update({ where: { id: 1 }, data: { stampUrl: url } });
    } else {
      await prisma.companySettings.create({ data: { stampUrl: url } });
    }

    return NextResponse.json({ url });
  } catch (err) {
    // eslint-disable-next-line no-console
    console.error("POST /api/settings/stamp error:", err);
