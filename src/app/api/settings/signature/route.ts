...
export async function POST(req: Request) {
  try {
    ...
    if (!allowed.includes(file.type)) {
      return NextResponse.json({ error: "Format i pavlefshëm. Lejohen: JPG, PNG, WEBP" }, { status: 400 });
    }
    ...
    if (existing) {
      await prisma.companySettings.update({ where: { id: 1 }, data: { signatureUrl: url } });
    } else {
      await prisma.companySettings.create({ data: { signatureUrl: url } });
    }

    return NextResponse.json({ url });
  } catch (err) {
    // eslint-disable-next-line no-console
    console.error("POST /api/settings/signature error:", err);
